...
dir is refused at boot with the holder's PID - an exclusive `flock` on a
machine-local lockfile keyed by the config dir (under forge's app-support
dir, NOT the synced config dir, so Syncthing's rename-on-sync can't swap the
lock inode out from under a running forge on another Mac), released
automatically on exit/crash; the `WorkspaceError::AlreadyRunning` hard-fail
