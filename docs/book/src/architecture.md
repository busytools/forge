...
| `forge-providers` | One backend per provider token: credential resolution, the usage probe's HTTP and payload mapping, billing shape. Depends on forge-primitives only; keychain, CLI-spawn refresh and TLS-trust plumbing arrive through the host port forge-agent implements. |
