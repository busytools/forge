...
| Model | The canonical model the session runs on, exactly as `forge.toml` declares it - the project's `model`, or the name picked at `/model`. A project that declares no model has nothing to pin, so its row shows whatever the CLI reports. No humanised label, no `[1m]` marker, no account slug | |
