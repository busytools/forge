...
| Model | The canonical model the session runs on, exactly as `forge.toml` declares it - the project's `model`, or the name picked at `/model`. Never the id the CLI resolves on its own, and no `[1m]` marker or account slug | |
