...
Every count describes the tab's INSTALLED rows - the pane keeps the registry-backed installs and the marketplace catalog as two separate row streams, and only the installed one feeds the numbers. A machine with 19 plugins and 74 skills shows `Installed 19` and `Skills 74`, never the catalog's size.
...
- **Installed** - one row per registry-backed plugin, plus its always-on token cost when `claude plugin details` has reported one (fetched once per installed version, then cached). The marketplace catalog never renders here: an available plugin is not an install. A registered plugin whose install directory vanished renders on this tab with its failure reason.
- **Skills / Agents / Commands** - one row per INSTALLED component of that kind, sourced by its plugin. The marketplace catalog's components of the same kind render only behind the Available toggle, dim, after the installed rows.
...
The action row carries `Available (a) +N` on the component tabs, where `+N` is the catalog's row count for that tab. Pressing <kbd>a</kbd> appends the available stream's rows after the installed ones: dim names, blue statuses, a plain `Install` action - visually secondary, exactly like the marketplace rows in the catalog. The toggle is per-pane state, off by default, and never touches the Installed tab: the Installed tab is the registry-backed tier alone.
...
- The inventory scan reads the plugin cache, the plugin registry and the marketplace manifests straight off disk. It runs when the page opens and on session change (the pane's five-second TTL), never spawning `claude` - the manual <kbd>r</kbd> and post-action refreshes keep the full CLI path - and first paint renders from the cached snapshot: the pane shows its loading copy while the first scan flies rather than blocking on a CLI spawn.
