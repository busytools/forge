...
- **MCPs** - one row per live session server over the shared grammar: the state glyph from the connection status, the server name, the scope as source - whatever the CLI reports (`project`, `user`, `local`, `claudeai`, `managed`, or `dynamic` for a plugin-provided server), falling back to `session` when it reports none - the status in the status column (`connected`, `needs auth`, `pending`, `disabled`, `failed`), the transport as a bracketed badge, and the dim detail carrying the tool count and the command or URL (the server's reported name and version when it reports them; on a failed server the error replaces the summary). There is no summary band: the rows carry the state and the tab chip carries the count. Enter opens the per-server details overlay - status, enabled, scope, transport, tools, the server's configuration, and the actions (Refresh, plus Reconnect and Disable on an enabled server or Enable on a disabled one) - and Esc closes it.
