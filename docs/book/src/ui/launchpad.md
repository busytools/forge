...
A bailed chip appends its reason after the name - `- rate limited (retry after 3600s)`, `- unauthorized` for auth classes. While any account is non-terminal every project row downgrades to a blocked click and the footer reads `⏳ loading accounts...`; once the gate lifts rows un-dim and Enter resumes. When a project has opted into the gateway (the `[projects.<name>]` `gateway` key), the gate also covers the listener: with the port taken the footer reads `⛔ gateway failed: ...` naming the port and the error instead, and it will not open this run - that project's every spawned session base URL names that port, so forge restarts rather than pointing children at an address nothing serves. Spawn attempts for opted-in projects made while the gate is shut (boot-wave, workers, crons) are refused at the spawn entries with the reason attached. With no opted-in project the listener's failure does not gate anything: preflight's Gateway row names it, and every project keeps the direct path.
