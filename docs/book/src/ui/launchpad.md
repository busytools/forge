...
- Each project row carries a trailing `(<account>)` chip showing the account the walk picks for it, which is what a spawn would land on; one worker row nests under the project per persisted dynamic worker the boot re-spawn wave would still start, each with the same four elements - lifecycle glyph, name, the same chip, and the right-aligned activity column (<code>&mdash;</code> for a worker that is neither spawning nor failed). The walk answers by org and model rather than by session, so every row of one project carries the same chip. A persisted row with no live worker shows `○`. A worker whose session bucket has not landed falls back to its own liveness, the same answer the Projects pane gives for it: `⠋` while it is spawning, `●` once it is running. A worker with a live backgrounded task spins instead of showing the idle `●`, the same promotion the Projects pane applies to that worker. The activity cell reads <code>&mdash;</code> either way, because a worker's worktree is keyed apart from its project so no per-worker timestamp reaches the column - which is why a running worker's row has always paired its spinner with <code>&mdash;</code>. Chip color tracks the picked account's runtime state: dim = normal; warning yellow, no glyph = at the usage cap (the session still spawns but will throttle) or bailed on a transient failure; error red + `⚠` = bailed on an auth failure. A row is skipped outright rather than offered as one that cannot spawn when a resume would have nowhere to start: the wave skips it and the pane does not nest it. The row itself is kept, so restoring the worktree brings the worker back. A row with no stored id is not that case - it re-spawns fresh in the project root, where claude creates its own worktree. Worker rows are info-only - selection stays on the project row.
