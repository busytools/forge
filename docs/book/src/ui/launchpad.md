...
- **per-project pool** - A project's pool intersects its `[[orgs]].accounts` allow-list with the set of `Ready` accounts, then prefers the accounts not at the usage cap - a session lands on a saturated (100%) account only when every candidate is capped. When no ready candidate exists, the pool rebuilds from the project's `Bailed` accounts (a rate-limited probe hits the usage endpoint, not inference, so spawning on one is legitimate). Only when the allow-list matches neither set does the project render a DIM `no usable accounts` sub-row and stay unclickable after the gate lifts.
