...
- Each project row carries a trailing `(<account>)` chip showing the account the walk picks for it, which is what a spawn would land on; one worker row nests under the project per persisted dynamic worker that still has a directory to run in, each with the same four elements - lifecycle glyph, name, the same chip, and the right-aligned activity column (<code>&mdash;</code> for a worker that is neither spawning nor failed). The walk answers by org and model rather than by session, so every row of one project carries the same chip. A persisted row with no live worker shows `○`. A worker whose session bucket has not landed falls back to its own liveness, the same answer the Projects pane gives for it: `⠋` while it is spawning, `●` once it is running. Chip color tracks the picked account's runtime state: dim = normal; warning yellow, no glyph = at the usage cap (the session still spawns but will throttle) or bailed on a transient failure; error red + `⚠` = bailed on an auth failure. A worker whose worktree is gone is skipped outright rather than offered as a row that cannot spawn, and the boot re-spawn wave skips it too; the row itself is kept, so restoring the worktree brings the worker back. Worker rows are info-only - selection stays on the project row.
