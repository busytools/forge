...
**Slack approval** covers four actions and uses the same picker: `slack__post` (send a message), `slack__edit` (replace a message body, `Do not post` focused as the safe answer), `slack__edit` with `delete: true` (remove a message), and `slack__react` (add or remove a reaction). The header names the workspace, conversation or thread, and the body prints exactly what would be sent, replaced or deleted, because that is what the approval is for. Only the approving option sends anything: <kbd>Esc</kbd>, quitting, an unanswered prompt, or the asking session going away all reject, and the message is left untouched. The prompt is queued on the session that asked, not the focused one.
