...
A comment whose turns span several rounds is listed under every review it has a turn in; the totals footer counts each comment once. <kbd>↑↓</kbd> move, <kbd>Enter</kbd> jumps the diff to the selected review's first comment, <kbd>l</kbd> / <kbd>Esc</kbd> / a click away close. Overviews render only here, never inline in the diff.
