...
sed -e "s/__USERNAME__/$USERNAME/g" -e "s/__WG_IP__/$WG_IP/g" \
    launchd/dev.vedhavyas.forge-daemon.plist \
    | sudo tee /Library/LaunchDaemons/dev.vedhavyas.forge-daemon.plist > /dev/null
sudo chown root:wheel /Library/LaunchDaemons/dev.vedhavyas.forge-daemon.plist
sudo chmod 644         /Library/LaunchDaemons/dev.vedhavyas.forge-daemon.plist

# Bootstrap + enable
sudo launchctl bootstrap system /Library/LaunchDaemons/dev.vedhavyas.forge-daemon.plist
sudo launchctl enable system/dev.vedhavyas.forge-daemon

# Verify
launchctl print system/dev.vedhavyas.forge-daemon
forge-daemon status
```
