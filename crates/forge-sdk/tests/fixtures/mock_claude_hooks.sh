#!/usr/bin/env bash
# Mock: emits init, reads a user prompt, then emits a PreToolUse hook
# request, reads the response, echoes what command the hook allowed.
...
# PreToolUse hook firing. callback_id="hook_0" assumes a single PreToolUse
# registration, matching Hooks::mint_registry ordering.
...
# Extract updatedInput.tool_input.command from the nested
# hookSpecificOutput.updatedInput wrapper, else fall back to original.
...
# Branch on whether the hook blocked the call.
