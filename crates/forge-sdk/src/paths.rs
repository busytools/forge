...
#[cfg(target_os = "macos")]
fn load_oauth_credentials_from_keychain(config_dir: &std::path::Path) -> Option<OauthCredentials> {
...
mod tests {
    ...
    fn keychain_service_name_uses_sha256_prefix_of_config_dir_string() {
        ...
        assert_eq!(
            keychain_service_name(std::path::Path::new("/Users/vedhavyas/.claude-nf")),
            "Claude Code-credentials-7a8e7f2e"
        );
        assert_eq!(
            keychain_service_name(std::path::Path::new("/Users/vedhavyas/.claude-granite")),
            "Claude Code-credentials-0ed1d9d0"
        );
        assert_eq!(
            keychain_service_name(std::path::Path::new("/Users/vedhavyas/.claude-subspace")),
