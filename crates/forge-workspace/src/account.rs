...
mod tests {
    ...
    fn lru_picks_oldest_first() {
        ...
        last_used.insert("Gateway".to_owned(), Some(later));
        last_used.insert("Stargate".to_owned(), Some(earlier));

        let mut map = AccountStateMap::new(
            &[make_account("Stargate"), make_account("Gateway")],
    ...
    fn lru_none_sorts_before_some() {
        ...
        last_used.insert("Stargate".to_owned(), Some(later));
        // Gateway has never been used (no entry).
    ...
    fn lru_tie_breaks_alphabetically_when_both_never_used() {
        ...
        let (picked, _) = map.pick_next(now);
        // Gateway < Stargate alphabetically.
        assert_eq!(picked.0, "Gateway");
    }
    ...
    fn round_robin_cycles_in_definition_order() {
        ...
        assert_eq!(a.0, "Stargate");
        assert_eq!(b.0, "Gateway");
        assert_eq!(c.0, "Stargate");
    }
    ...
    fn round_robin_resumes_from_persisted_next() {
        let mut map = AccountStateMap::new(
            &[make_account("Stargate"), make_account("Gateway")],
            SelectionPolicy::RoundRobin,
            Some(1), // resume at index 1 = Gateway
            &std::collections::HashMap::new(),
        );
        let now = SystemTime::UNIX_EPOCH;
        let (picked, _) = map.pick_next(now);
    ...
    fn pick_returns_correct_config_dir() {
        ...
        assert_eq!(dir, PathBuf::from("/fake/Stargate"));
    }
}
