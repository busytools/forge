...
/// register for both kinds - unlike `peers`, which is lead-only. What each
...
/// made them, and tasks to the caller's project. A worker is exactly the
/// session a review nudge lands on, so it needs `review__*`.
///
