...
mod tests {
    ...
    fn seed_migrates_state_toml_and_marks() {
        ...
        write_state_toml(cfg.path(), base.path(), Some(SpinnerStyle::Ember), &[("Gateway", 42.0)]);
        ...
        assert!(
            account_usage(&db).expect("usage").contains_key("Gateway"),
    ...
    fn seed_migrates_present_fields_only() {
        ...
        write_state_toml(cfg.path(), base.path(), None, &[("Gateway", 42.0)]);
