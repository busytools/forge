...
mod tests {
    ...
    fn validate_label_accepts_namespaced_labels() {
        assert!(validate_label("hub-modules/researcher").is_ok());
    ...
    fn role_dir_joins_label_under_forge_team_root() {
        ...
        let nested = role_dir("hub-modules/researcher").expect("namespaced label resolves");
        assert_eq!(nested, root.join("hub-modules").join("researcher"));
    }
