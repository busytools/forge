...
mod tests {
    ...
    #[test]
    fn build_server_registers_whoami() {
        let mock = MockWorkspaceFacade::new();
        let facade = mock.into_arc();
        let server = build_server(facade, fake_key("test"));
        // McpServer doesn't expose tool listing publicly; assert
        // construction succeeds (Debug should mention `peers__whoami`).
        let debug = format!("{server:?}");
        assert!(
            debug.contains("peers__whoami"),
            "build_server must include peers__whoami; debug: {debug}",
        );
    }
}
