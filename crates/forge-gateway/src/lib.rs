...
/// Why a probe did not produce a snapshot: a credential miss, a wire
/// error class from the round-trip, or a 200 whose body maps to
/// nothing.
...
pub trait ProviderHost: Send + Sync {
    ...
    /// roots applied, and `idle_timeout` as a read bound that resets on
    /// every chunk. A streamed inference response has no known length,
    /// so the total deadline [`ProviderHost::http_client`] bakes in
    /// would cut a healthy long turn; an idle bound still ends one
    /// whose upstream has stopped producing bytes.
...
pub trait ProviderBackend: Send + Sync {
    ...
    fn repair(&self, _account: &AccountEnv<'_>, err: &ProbeError) -> RepairAction {
        match err {
            ...
            ProbeError::Fetch(_) | ProbeError::Unmappable(_) => {
