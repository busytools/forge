...
/// 24-column pane, so the paint dropped the last two hex digits of the
/// sha with no ellipsis and nothing to show it had happened.
///
/// Asserted against the painted buffer and against the RIGHT GUTTER,
/// not the pane edge. A "fits the pane" assertion cannot fail here: the
/// buffer is only 24 cells wide, so a clipped row and a fitting row are
/// the same 24 characters. The gutter is what distinguishes a row built
/// to fit from one the paint cut off, and it is the budget the claude
/// row directly below already keeps.
#[test]
fn the_forge_version_row_clears_the_medium_pane_gutter() {
    // 24 = PANE_WIDTH_MEDIUM, less a 1-col PANEL_RIGHT_GUTTER.
    ...
    let lines = render_to_lines(&mut app, &projects, 24, 30);
    let row = lines
        .iter()
        .find(|l| l.trim_start().starts_with("forge "))
        .unwrap_or_else(|| panic!("the forge version row renders, got:\n{}", lines.join("\n")));

    assert!(
        row.chars().count() <= BUDGET,
        "the version row leaves the panel right gutter clear at Medium; got {} chars: [{row}]",
        row.chars().count(),
    );
    // Not `contains('+')`: an ellipsis fix would satisfy that while
    // leaving the sha unusable. What matters is that the shortened sha is
    // a clean PREFIX of the real one, so it can still be matched against
    // a build - which is the whole reason the stamp carries a sha.
    let painted_sha = row.rsplit('+').next().expect("row splits");
    let full_sha = forge_tui::FORGE_VERSION_SHORT.rsplit('+').next().expect("stamp splits");
    assert!(
    ...
    // Wide has room, so the fix must not reach it: the full stamp renders
    // there exactly as before. This is the negative control - without it a
    // budget applied at every tier would look identical at Medium.
    let wide = render_to_lines(&mut app, &projects, 32, 30);
    let wide_row =
        wide.iter().find(|l| l.trim_start().starts_with("forge ")).unwrap_or_else(|| {
            panic!("the forge version row renders at Wide, got:\n{}", wide.join("\n"))
        });
    assert!(
        wide_row.ends_with(&format!("v{}", forge_tui::FORGE_VERSION_SHORT)),
