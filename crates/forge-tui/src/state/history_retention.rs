...
use crate::state::tool_call_info::{InlinePermission, InlineQuestion, ToolCallInfo};
use crate::state::types::{HistoryRetentionStats, MessageUsage};
...
impl crate::state::app::App {
    ...
    pub(super) fn rebuild_tool_indices_and_terminal_refs(&mut self) {
        ...
        for msg in &self.messages {
            for block in &msg.blocks {
                ...
                match self.tool_call_scopes.get(&tc.id) {
                    Some(crate::state::types::ToolCallScope::SubagentRoot) => {
                        self.active_task_ids.insert(tc.id.clone());
                    }
                    Some(
                        crate::state::types::ToolCallScope::SubagentChild { .. }
                        | crate::state::types::ToolCallScope::MainAgent,
                    )
                    | None => {}
                }
            }
        ...
        if let Some(first_id) = self.pending_interaction_ids.first().cloned() {
            ...
            self.release_focus_target(crate::state::focus::FocusTarget::Permission);
        }
