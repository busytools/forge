#![allow(
    ...
    reason = "lifted upstream from claude-code-rust; consumers come in later phases"
)]
//! Wire data types lifted from claude-code-rust `agent/model.rs`.
//!
//! These represent the SDK-side event surface: `SessionUpdate` is the
//! canonical event the UI consumes, with `ToolCall`, `ToolCallContent`,
//! `Diff`, etc. as nested types. The forge wire adapter (Phase 3)
//! converts forge-daemon `serde_json::Value` events into these types.
...
pub enum SessionUpdate {
    ...
    ModeStateUpdate(crate::state::types::ModeState),
