...
pub fn reset(app: &mut App) {
    ...
    app.file_index.respect_gitignore = app.respect_gitignore_effective();
...
pub fn restart(app: &mut App) {
    ...
    let respect_gitignore = app.respect_gitignore_effective();
...
fn refresh_after_mutation(app: &mut App) {
    if app.mention.is_some() {
        crate::state::mention::refresh_from_file_index(app);
    }
