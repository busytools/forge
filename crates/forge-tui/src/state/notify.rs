...
/// Preferred notification channel. Lifted inline from upstream's
/// `app/config/mod.rs` since the rest of `config/*` is not lifted.
/// Daemon will push the resolved value via `config_options` when the
/// wire surface ships it.
