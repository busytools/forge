//! The shared composer chrome: the thick orange border, the prompt
//! glyph, and the placeholder and cursor styles, plus a Line face that
//! embeds titles and builds bordered rows for surfaces that render
//! into a scrollback Vec. The chat composer is the template; every
//! text surface switches the capabilities it needs.
...
/// what they do not carry. Titles embed through the Line face's
/// [`top_border`], which the widget-faced surfaces never need.
...
impl ComposerChrome {
    ...
    pub(crate) fn chat() -> Self {
        Self { prompt_glyph: true, placeholder: "Type a message..." }
    }

    /// The thick orange border every surface draws; `border_fg` is the
    /// resolved colour, so a live dictate take can hand its own over.
    pub(crate) fn border_block(border_fg: Color) -> Block<'static> {
        let style = Style::default().fg(border_fg).add_modifier(Modifier::BOLD);
        Block::default().borders(Borders::ALL).border_type(BorderType::Thick).border_style(style)
    }
...
mod tests {
    ...
    #[test]
    fn the_border_block_carries_thick_bold_corners() {
        let mut terminal =
            ratatui::Terminal::new(ratatui::backend::TestBackend::new(12, 3)).expect("terminal");
        terminal
            .draw(|frame| {
                frame.render_widget(ComposerChrome::border_block(theme::RUST_ORANGE), frame.area());
            })
            .expect("draw");
    ...
    fn the_prompt_line_is_the_orange_glyph_and_a_space() {
        ...
        assert!(
            ComposerChrome { prompt_glyph: false, placeholder: "" }.prompt_line().is_none(),
