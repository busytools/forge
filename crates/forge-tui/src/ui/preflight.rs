...
//! degraded: its row names the failure, the walk keeps it as a last
//! resort, and the pollers keep re-probing it, so holding the screen
//! waits on nothing that could change it.
//!
...
/// walk keeps it as a last resort and the pollers keep re-probing it,
/// so the screen would be waiting on nothing that could change it.
