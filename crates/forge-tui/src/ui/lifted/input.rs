...
use crate::state::focus::FocusOwner;
use crate::state::input::parse_paste_placeholder_ranges;
use crate::state::types::AppStatus;
use crate::state::{mention, subagent};
use crate::ui::theme;
use ratatui::Frame;
...
pub fn render(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    if let Some(sel) = app.selection
        && sel.kind == crate::state::types::SelectionKind::Input
    {
...
fn apply_textarea_highlights(textarea: &mut TextArea<'_>, lines: &[String]) {
    ...
    for (row, line) in lines.iter().enumerate() {
        ...
        for (start, end, _) in crate::state::clipboard_image::find_image_badge_spans(line) {
            textarea.custom_highlight(
                ((row, start), (row, end)),
                image_badge_style,
...
struct SelectionOverlay {
    selection: crate::state::types::SelectionState,
}
