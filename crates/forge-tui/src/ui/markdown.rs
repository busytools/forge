...
//! Safe markdown renderer lifted from `claude-code-rust`.
//!
//! Wraps `tui_markdown::from_str` in a panic catcher that falls back
//! to plain-text rendering — important because `tui_markdown` has
//! known panic paths on degenerate inputs (nested checklists,
//! malformed link syntax, ZWJ sequences).
