...
pub(crate) mod tests {
    ...
    fn mcp_server_rows_do_not_wrap_at_40_cols() {
        ...
        let name_span = name_line
            ...
            .expect("name span");
        assert_eq!(name_span.style.fg, None, "server name renders in default fg");
        assert!(
    ...
    fn app_with_git_gate(repo_gate: RepoGate) -> App {
        let mut app = App::test_default();
        let key = forge_workspace::SessionSlot::from_str_for_test("inspector-git-test");
    ...
    fn app_with_waiting_session(name: &str) -> App {
        let mut app = App::test_default();
        let key = forge_workspace::SessionSlot::from_str_for_test(name);
    ...
    fn attention_band_renders_a_waiting_review_replies_row() {
        let mut app = App::test_default();
        let key = forge_workspace::SessionSlot::from_str_for_test("reviewer");
    ...
    fn attention_detail_formats_kind_tool_and_wait() {
        ...
        let perm = AttentionEntry {
            session_key: forge_workspace::SessionSlot::from_str_for_test("p"),
    ...
    fn attention_detail_formats_failure_with_and_without_status() {
        ...
        let entry = AttentionEntry {
            session_key: forge_workspace::SessionSlot::from_str_for_test("f"),
            name: "gateway-backend".to_owned(),
            role: None,
    ...
    fn attention_row_stamps_clickable_jump_target() {
        ...
        let mut app = app_with_waiting_session("gateway-backend");
        let bg = forge_workspace::SessionSlot::from_str_for_test("gateway-backend");
        assert_ne!(
    ...
    fn attention_band_caps_rows_clamps_body_and_skips_clipped_rows() {
        ...
        let mut app = App::test_default();
        for i in 0..6 {
            let key = forge_workspace::SessionSlot::from_str_for_test(format!("bg-{i}"));
