//! Logging target constants used by `tracing` macro `target:` fields.
//!
//! Stub of upstream's `src/logging.rs` — only the `targets` submodule
//! is lifted today. The full subscriber + file-rotation machinery
//! lifts when a TUI binary needs it; for now `tracing_subscriber`'s
//! defaults are enough.
