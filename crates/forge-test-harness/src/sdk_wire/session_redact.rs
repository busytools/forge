...
/// it replaces only the home prefix, and `~/.claude-nf` has no prefix at
...
mod tests {
    ...
    fn profile_dir_is_redacted_home_relative_and_absolute() {
        let v = redact_one(
            &json!({"text": "see ~/.claude-nf/CLAUDE.md and /Users/alexandra/.claude-granite1/x"})
                .to_string(),
        );
        assert_eq!(
            v["text"],
    ...
    fn object_keys_get_every_rule_not_just_the_path_one() {
        let mut v = json!({"~/.claude-nf/settings.json": 1});
        scrub_paths_recursive(&mut v);
    ...
    fn a_line_that_is_not_json_still_gets_the_text_rules() {
        let line = "node:internal/fs: ENOENT /Users/alexandra/.claude-nf/settings.json";
        assert_eq!(
